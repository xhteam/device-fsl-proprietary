...
def find_addr2line(version):
    ...
    if sys.platform.startswith("linux"):
        ...
        output = get_process_output(["find", android_src_root+os.sep+"prebuilts", "-iregex", debugger_pattern])
    if len(output) > 0:
...
def find_debugger(version):
    ...
    if sys.platform.startswith("linux"):
        ...
        p = subprocess.Popen(["find", android_src_root+os.sep+"prebuilts", "-iregex", debugger_pattern], stdout=subprocess.PIPE)
        p.wait()
    output = p.stdout.readlines()
    if len(output) > 0:
...
if __name__ == "__main__":
    ...
    KEY_ANDROID_PRODUCT = "TARGET_PRODUCT"
    default_android_src_root = ""
    default_android_product = "generic"
    if os.environ.has_key(KEY_ANDROID_SRC_ROOT):
        default_android_src_root = os.environ[KEY_ANDROID_SRC_ROOT]
    if os.environ.has_key(KEY_ANDROID_PRODUCT):
        default_android_product = os.environ[KEY_ANDROID_PRODUCT]
    ...
    opt_parser.add_option("", "--debugger-version", dest="debugger_version", default="4.6", 
            help="specify a debugger version, e.g., 4.4.0, 4.2, 4. Will use %default if not specified")
    opt_parser.add_option("", "--product-name", dest="product_name", default=default_android_product, 
