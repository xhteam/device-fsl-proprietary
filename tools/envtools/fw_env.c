...
#ifdef DEBUG
		printf ("Write total 0x%x bytes count[0x%x]\n", write_total,count);
#endif
...
#ifdef DEBUG
		printf ("blocklen[0x%x],blockstart=0x%x erasesize[0x%x] top_of_range[0x%x]\n",blocklen,blockstart,erasesize,top_of_range);
#endif
...
int fw_env_open(void)
{
	...
	#if !defined(CONFIG_FILE)&&!defined(HAVE_REDUND)
	if(strstr(DEVICE1_NAME,"mmcblk0boot0")){
		int fd = open("/sys/class/block/mmcblk0boot0/force_ro", O_RDWR );
		if( fd >= 0 ){
			char buf[48];
			int len;
			len=sprintf(buf,"0\n");
